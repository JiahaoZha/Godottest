...
class Simulator:
    ...
    def handle_event(self, event):
        ...
        if event.event_type == Event.ENQUEUE:
            ...
            if event.packet in event.target_node.output_queue:
            
        elif event.event_type == Event.PROPAGATE:
            self.schedule_event_after
            
        elif event.event_type == Event.RECEIVE:
            print(f"Packet {event.packet.packet_id} received by {event.target_node.node_id}")
                if event.target_node.node_id != event.packet.destination.node_id:
                    self.schedule_event_after(Event(Event.ENQUEUE, event.target_node, event.packet), 0)            
